...
class Database(){
    ...
    fun init(){
        ...
        customerAddHistory("peter", Review("peter", brew, "It's brew"))

        customerAdd("admin", "admin", "admin")
        customerAddHistory("admin", Review("peter", dodgerS, "It's dodgers"))
    ...
    {
        Log.d("Tag", "Person exist? Name:${userBase["peter"]?.userName} pass ${userBase["peter"]?.password} history ${userBase["peter"]?.history?.get(0)?.comments}")
    }
